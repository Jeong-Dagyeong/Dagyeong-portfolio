      ...
      <section className="mb-12">
        ...
        <div className="mt-8 text-left">
          ...
          <div className="space-y-4 mt-4 text-gray-300 leading-relaxed">
            ...
            <p className="text-l font-semibold text-yellow-200">[해결]</p>
            백엔드와 협업하여 다양한 방식으로 테스트를 진행한 끝에, 프론트엔드에서 react-image-file-resizer 라이브러리를
            활용해 이미지 용량을 줄여 전송하는 방식을 적용했습니다. 이미지 크기 최적화와 전송 방식 개선을 통해 백엔드
            부하를 줄이고 처리 안정성을 확보할 수 있었습니다.
            <p className="text-l font-semibold text-yellow-200">[회고]</p>
          ...
          <div className="space-y-4 mt-4 text-gray-300 leading-relaxed">
            <p className="text-l font-semibold text-yellow-200">[문제점]</p>
            처음에는 각 컴포넌트에서 채팅 관련 로직을 분산해 관리했지만, 유지보수가 어렵고 비효율적이라는 판단 하에 상위
            컴포넌트에서 사용자 입력을 처리하도록 구조를 변경하였습니다. 이후 말풍선이 두 번씩 출력되거나, 입력이
            전송되지 않는 등 다양한 채팅 입력 오류가 발생했으며, 이는 return 누락, 상태관리(setState)의 불일치 등으로
            인한 것이었습니다.
            <p className="text-l font-semibold text-yellow-200">[해결]</p>
            문제의 원인을 함수 흐름과 상태관리 방식에서 찾았고, params와 useState를 혼용하던 구조를 setForm으로 통일하여
            상태 불일치를 해소하였습니다. 또한, 채팅 전송 구조를 리팩토링하여, 사용자 입력을 저장한 후 API 호출 시 함께
            전송하고 초기화하는 방식으로 구현하여 안정적인 전송 흐름을 확보했습니다.
            <p className="text-l font-semibold text-yellow-200">[회고]</p>이 경험을 통해 함수의 흐름과 상태관리 일관성의
            중요성을 실감했습니다. 공식 문서의 권장 방식을 따르되, 실제 서비스 흐름에 맞는 구조를 스스로 고민하고
            구현해야 한다는 점을 배웠습니다.
          </div>
        </div>
      </section>
